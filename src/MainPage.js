...
class MainPage extends Component {
  ...
  render() {
      ...
      if (response.ok) {
			...
			//console.log("devices int::"+devices);
			this.setState({ devicesJson: devices });
			
			fetch('https://tracking.greenorange.in/api/positions').then(response => {
				if (response.ok) {
	   ...
	   for (let i = 0; i < this.state.devicesJson.length; i++) {
		   ...
		   for (let j = 0; j < this.state.positionJson.length; j++) {
			   ...
			   if(this.state.devicesJson[i].id == this.state.positionJson[j].deviceId){
				  // console.log("pushed");
				   tableData.push({name: this.state.devicesJson[i].name, status: this.state.devicesJson[i].status, speedLimit: this.state.devicesJson[i].attributes.speedLimit, ignition: this.state.positionJson[j].attributes.ignition, latitude: this.state.positionJson[j].latitude, longitude: this.state.positionJson[j].longitude});
			   }
		   }
		   if(tableData.length == i){
			   tableData.push({name: this.state.devicesJson[i].name, status: this.state.devicesJson[i].status, speedLimit: this.state.devicesJson[i].attributes.speedLimit, ignition: '', latitude: '', longitude: ''});
		   }
		   //this.setState({ rowData: [{make: this.state.devicesJson[i].id, model: "Celica", price: 35000},{make: "Toyota", model: "Celica", price: 35000}] }) ;
	   }
